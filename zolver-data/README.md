...
![파이프라인 플로우](./docs/pipeline_flow.png)
![업로드 시퀀스](./docs/upload_sequence.png)
...
| 검증 항목 | 설명 | 에러 코드 |
|---|---|---|
| 파일 확장자 | `.xlsx` 여부 확인 | R101 |
| 파일 용량 | 허용 범위 내 용량인지 확인 | R102 |
| 수정자 메타데이터 | 화이트리스트 키워드 스캔, 위변조 탐지 | R103 |
...
| 검증 항목 | 설명 | 에러 코드 |
|---|---|---|
| 좌표 구조 검증 | 헤더 텍스트가 고정 좌표에 존재하는지 확인 | R201 |
| 학번 일치 | 성적표 내 학번과 로그인 유저 학번 대조 | R202 |
